...
public interface DiagnosticSpecificationMapper {
    ...
    @Select("select * from diagnostic_specification where patient_id=#{patientId}")
    List<DiagnosticSpecification> findDiagnosticSpecificationforpat(int patientId);

    @Select("select * from diagnostic_specification where doctor_id=#{doctorId}")
    List<DiagnosticSpecification> findDiagnosticSpecificationfordoc(int doctorId);
    ...
    @Options(useGeneratedKeys = true,keyProperty = "diagnosticSpecificationId", keyColumn = "diagnostic_specification_id")
