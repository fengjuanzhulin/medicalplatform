...
public class UserAdminController {
    ...
    public JsonData deleteUserById(@RequestParam(value = "user_id", required = true) int userId) {
        int y=0;
        String userName = userService.showUserName(userId);
        int i = userService.deleteUserById(userId);
        int id = userService0.findId(userName);
        int i2 = userService0.deleteUserRoleById(id);
        int i1 = userService0.deleteUserByName(userName);
        if (i==1&&i1==1&&i2==1){
            y=1;
        }
        return y==1?JsonData.buildSuccess():JsonData.buildError(""+i+i1+i2);
    }
