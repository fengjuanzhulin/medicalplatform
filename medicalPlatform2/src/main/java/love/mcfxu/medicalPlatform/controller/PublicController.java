...
public class PublicController {
    ...
    public JsonData needLogin(){
        ...
        return JsonData.buildSuccess("请使用相应的账号登录",-10000);
    ...
    public JsonData notPermit(){
        ...
        return JsonData.buildSuccess("拒绝访问，没权限",-10001);
    }
    ...
    public JsonData register(@RequestBody Map<String,String> patientInfo ){
        ...
        if ((patientInfo.get("user_role").equals("2"))&&(userService.findByUserName(user_name)==null)){
            ...
            return rows == 0 ? JsonData.buildSuccess(): JsonData.buildError("注册失败，请重试",-1);}
        else {
            return JsonData.buildError("账号注册不合规，请更换角色定位",-1);
        }
    }
    ...
    public JsonData login(@RequestBody UserQuery userQuery, HttpServletRequest request, HttpServletResponse response){
        ...
        try {
            ...
            User user = userService0.findAllUserInfoByUsername(primaryPrincipal.toString());
            User user0 = userService0.findAllUserInfoByUsername(user.getUsername());

//            User user = (User)subject.getPrincipal();
            String username = user0.getUsername();
            String password = user0.getPassword();

            UserEntity userInfo = userService.findUserInfo(username, password);
...
//            session.setAttribute("user_role",userRole);
            ...
            return JsonData.buildError("账号或者密码错误",-1);
